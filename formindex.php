...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if(isset($_POST['nom']) && isset($_POST['email']) && isset($_POST['activite']) && isset($_POST['message'])) {
        $nom = filter_input(INPUT_POST, 'nom', FILTER_SANITIZE_STRING);
        $email = filter_input(INPUT_POST, 'email', FILTER_SANITIZE_EMAIL);
        $activite = filter_input(INPUT_POST, 'activite', FILTER_SANITIZE_STRING);
        $message = filter_input(INPUT_POST, 'message', FILTER_SANITIZE_STRING);

        // Validation des données soumises
        if(empty($nom) || empty($email) || empty($activite) || empty($message)) {
        ...
        $stmt = $pdo->prepare("INSERT INTO formindex (nom, email, activite, message) VALUES (:nom, :email, :activite, :message)");
